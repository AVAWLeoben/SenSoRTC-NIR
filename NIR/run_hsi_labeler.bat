...
REM ------------------------------------------------------------
REM Start HSI Labeler from the hsi-labeler conda environment.
REM Place this .bat file in the same folder as hsi_labeler.py.
REM ------------------------------------------------------------

cd /d "%~dp0"

set "ENV_NAME=hsi_labeler"
set "SCRIPT_PATH=%~dp0hsi_labeler.py"

if not exist "%SCRIPT_PATH%" (
    echo ERROR: Could not find hsi_labeler.py next to this batch file.
